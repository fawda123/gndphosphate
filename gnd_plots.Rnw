...
toplo <- filter(wq_dat, StationCode == 'BL') %>% 
  select(-StationCode) %>% 
  filter(turb < 250) %>%  # remove anom value
  ...
  mutate(variable = factor(variable, 
    ...
    labels = c('Total precipitation (cm/d)', 'Sea level elevation (m)', 'Salinity', 'Turbidty (NTU)', 'pH')
    )) %>% 
  rename(Event = TimeFrame)
...
p2 <- ggplot(toplo, aes(x = StationCode, y = value)) +
  ...
  ggtitle('(B) Within time frames')

grid.arrange(p1, p2, ncol = 1)
@

<<Fig7, fig.height = 5, fig.width = 7, echo = F>>=

dat <- wq_dat %>%
  filter(!StationCode %in% 'BH') %>%
  mutate(
    TimeFrame = gsub('^NI1$|^NI2$', 'NI', TimeFrame)
  ) %>%
  ...
  mutate(
    data = map(data, function(x){
      ...
      # delta datetimestamp, sal
      datetimestamp <- with(x, diff(datetimestamp)/2 + datetimestamp[-c(length(.))])
      dsal<- diff(x$sal) %>%
        sign

      # ph, sal diff
      sal <- diff(x$sal)/2 + x$sal[1:(nrow(x) - 1)]
      ph <- diff(x$ph)/2 + x$ph[1:(nrow(x) - 1)]

      out <- data.frame(datetimestamp, dsal, sal, ph) %>%
        gather('var', 'val', sal, ph) %>%
        na.omit %>%
        group_by(dsal, var) %>%
        summarise(
          avev = mean(val),
...
toplo1 <- dat %>%
  filter(TimeFrame %in% c('E1A', 'NI','E2A') & var %in% 'sal')

p1 <- ggplot(toplo1, aes(x = dsal, y = avev, group = TimeFrame)) +
  geom_errorbar(aes(ymin = lov, ymax = hiv, colour = TimeFrame), width = 0.3) +
  geom_line(aes(colour = TimeFrame)) +
  scale_x_discrete('Salinity change') +
  scale_y_continuous('Salinity (psu)') + 
  geom_point(aes(fill = TimeFrame), pch = 21, size = 3) +
...
toplo2 <- dat %>%
  filter(TimeFrame %in% c('E1A', 'NI','E2A') & var %in% 'ph')

p2 <- ggplot(toplo2, aes(x = dsal, y = avev, group = TimeFrame)) +
  geom_errorbar(aes(ymin = lov, ymax = hiv, colour = TimeFrame), width = 0.3) +
  geom_line(aes(colour = TimeFrame)) +
  scale_x_discrete('Salinity change') +
...
botlab <- expression(paste(Delta, 'sal / ', Delta, 't'))

grid.arrange(
  pleg, p1, p2, ncol = 1, heights = c(0.1, 1, 1), 
  bottom = grid::textGrob(botlab)
  )
@
...
nutnms <- c('orthophosphate (mg P/L)', 'ammonium (mg N/L)', 'nitrite/nitrate (mg N/L)', 'chlorophyll-a ($\\mu$g/L)')
names(nutnms) <- c('PO4F', 'NH4F', 'NO23F', 'CHLA_N')
      
for(nut in trnds$nutrient){
  ...
  cap <- paste0('Summary of ', nutnms[nut], ' trends for time periods with sufficient data.  Trends are based on seasonal Kendall tests, where $\\tau$ is the direction and strength of the trend, slope is the change per year, median is the median in the time period, and percent change is the slope divided by the median.')
  tab <- tab[, -c(1, 2)]
