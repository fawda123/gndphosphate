...
toplo <- filter(wq_dat, StationCode == 'BL') %>% 
  select(-StationCode, -depth) %>% 
  left_join(., met_proc, by = c('datetimestamp')) %>% 
...
# event dates
events <- c('2005-04-01 0:0', '2012-09-01 0:0')
events <- rep(as.POSIXct(events, format = '%Y-%m-%d %H:%M', tz = 'America/Regina'), 3)
...
# plots
p1 <- ggplot(toplo1, aes(x = datetimestamp, y = value)) + 
  geom_line() +
...
p2 <- ggplot(toplo2, aes(x = datetimestamp, y = value)) + 
  geom_line() +
...
toplo <- filter(nut_dat, StationCode != 'BN') %>% 
  ...
  summarise(logvalue = mean(logvalue, na.rm = TRUE)) %>% 
  spread(nutrient, logvalue)
pcres <- prcomp(toplo[, 3:6])
toplo <- data.frame(toplo, pcres$x)

p1 <- ggord(pcres, toplo$TimeFrame) +
  geom_text(data = toplo, aes(x = PC1, y = 0.07 + PC2, label = StationCode), colour = 'grey40', 
    size = 4)
p1
