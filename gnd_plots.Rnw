...
toplo <- filter(wq_dat, StationCode == 'BL') %>% 
  ...
  mutate(variable = factor(variable, 
    ...
    labels = c('Total precipitation (cm/d)', 'Sea level elevation (m)', 'Salinity', 'Turbidity (NTU)', 'pH')
    )) %>% 
  rename(Event = TimeFrame)
...
nutnms <- c('Chl-\\textit{a} ($\\mu$g/L)', 'NH$_4^+$ (mg N/L)')
names(nutnms) <- c('CHLA_N', 'NH4F')
