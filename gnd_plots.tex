...
\caption[Monthly phosphate time series at Bangs North (BN), Bangs Lake (BL), Point aux Chenes (PC), and Bayou Cumbest (BC) stations at Grand Bay]{Monthly phosphate time series at Bangs North (BN), Bangs Lake (BL), Point aux Chenes (PC), and Bayou Cumbest (BC) stations at Grand Bay. Distance from spill increases from top to bottom. Vertical green bars indicate a heavy rain event in April 2005 and hurricane Isaac in August 2012. E1A: event 1 acute, E1C: event 1 chronic, NI1: non-impact 1, E2A: event 2 acute, E2C: event 2 chronic, and NI2: non-impact 2.}\label{fig:Fig4}
\end{figure}
...
\caption[Diel phosphate and salinity data before and after Hurricane Isaac during the second spill event]{Diel phosphate and salinity data before and after Hurricane Isaac during the second spill event. Bayou Cumbest, is approximately 7 km (hydrologically) from the spill station and elevated phosphorus is observed with tidal influx (depth) following the event.}\label{fig:Fig5}
\end{figure}
...
\begin{table}[!tbp]
\caption{Station summaries of water quality (hourly) and nutrient (monthly) observations from 2005 to 2015.  Units are mg L$^{-1}$ for all variables, except Chl-\textit{a} as $\mu$g L$^{-1}$, DO\textit{sat} as percent, pH from 0-12, and salinity as psu. Nutrient summaries are based on maximum likelihood estimates for left-censored data and summary values less than the detection limit (0.01 mg/l) are estimated accordingly.  Stations are BC: Bayou Cumbest, BH: Bayou Heron (no nutrient data), BL: Bangs Lake, BN: Bangs North (no water quality data), and PC: Point aux Chenes.\label{tab:summtab}} 
\begin{center}
...
%latex.default(tab, file = "", rowlabel = "Time periods, stations",     caption = cap, caption.loc = "top", rgroup = rgrp, n.rgroup = nrgrp,     rowname = rows, colheads = c("$\\tau$", "slope", "median",         "\\% change"), label = paste0("tab:", nut, "trnd"))%
\begin{table}[!tbp]
...
\begin{tabular}{lllll}
\hline\hline
\multicolumn{1}{l}{Time periods, stations}&\multicolumn{1}{c}{$\tau$}&\multicolumn{1}{c}{slope}&\multicolumn{1}{c}{median}&\multicolumn{1}{c}{\% change}\tabularnewline
\hline
